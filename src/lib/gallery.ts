...
// _sm   = max 800px WebP for phones
// _web  = max 1600px WebP for desktop grids / retina
// _full = max 2400px WebP for lightbox / click-through views
...
export type Variant = "_sm" | "_web" | "_full";
...
// Responsive srcset (only includes variants that exist). Use together with a
// `sizes` attribute so each device downloads the smallest sufficient image.
export function variantSrcset(originalUrl: string): string | undefined {
  const sm = variantUrl(originalUrl, "_sm");
  const web = variantUrl(originalUrl, "_web");
  const parts: string[] = [];
  if (sm !== originalUrl) parts.push(`${sm} 800w`);
  if (web !== originalUrl) parts.push(`${web} 1600w`);
